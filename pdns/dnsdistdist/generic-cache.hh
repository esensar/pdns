...
public:
  ...
  void insert(const std::string& key, [[maybe_unused]] std::optional<LuaAny> value) override
  {
    insertKey(key);
  }
  ...
  bool getValue(const std::string& key, [[maybe_unused]] std::optional<LuaAny>& value) override
  {
    return contains(key);
  }
  ...
  [[nodiscard]] virtual const GenericCacheInterface<std::string, std::optional<LuaAny>>::Stats& getStats() const override
  {
    return d_stats;
  }
...
private:
  ...
  GenericCacheInterface<std::string, std::optional<LuaAny>>::Stats d_stats{"filter=\"bloom\""};
};
...
private:
  ...
  using stats_t = GenericCacheInterface<std::string, std::optional<LuaAny>>::Stats;
