...
void setupLuaBindingsCache(LuaContext& luaCtx)
{
  ...
  luaCtx.registerFunction<boost::optional<LuaAny> (std::shared_ptr<cache_t>::*)(const std::string&)>("get", [](std::shared_ptr<cache_t>& cache, const std::string& key) {
    boost::optional<LuaAny> result;
    if (!cache) {
      return result;
    }
